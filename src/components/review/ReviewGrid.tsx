            ...
            <table className={cn("w-full border-collapse text-xs", isMobile && "table-fixed")}>
              ...
              <thead>
                <tr className="bg-[#fafafa]">
                  ...
                  <th className={cn("z-10 bg-[#fafafa] px-0.5 py-2.5 text-left font-semibold text-muted-foreground border-b border-r border-[hsl(0,0%,0%,0.06)] text-[10px]", !isMobile && "sticky left-9 min-w-[120px] px-1")}>{filterConfig.show_exam_year_column ? "기출" : "주제"}</th>
                  {Array.from({ length: roundCount }, (_, i) => (
                  ...
                  <>
                    {sectionFilter === "all" && resultFilter === "off" && (
                       <tr key={`header-${group.type}`}>
                        <td colSpan={2 + roundCount} className="px-2 py-1.5 text-[10px] font-bold text-muted-foreground bg-[#fafafa] border-b border-[hsl(0,0%,0%,0.06)] uppercase tracking-wider">
